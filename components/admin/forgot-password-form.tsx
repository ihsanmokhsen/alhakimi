...
export function ForgotPasswordForm() {
  ...
  return (
    <section className="mx-auto w-full max-w-md rounded-[28px] border border-black/[0.06] bg-white p-6 shadow-[0_24px_90px_rgba(18,22,34,0.12)] sm:p-8">
      <div className="mb-8">
        <p className="text-[12px] font-black uppercase text-[#2563ff]">Reset access</p>
        <h2 className="mt-3 text-[34px] font-black leading-none tracking-normal text-black">Forgot Password</h2>
      </div>

      <form action={formAction} className="space-y-5">
        <div className="space-y-2">
          <label className="text-[12px] font-black uppercase text-black/42" htmlFor="email">
            Email
          </label>
          <input
        ...
        {state.success ? (
          <p className="rounded-2xl bg-[#2563ff]/10 px-4 py-3 text-sm font-bold text-[#2563ff]">{state.success}</p>
        ) : null}
