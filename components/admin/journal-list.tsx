    ...
    <div className="space-y-3">
      {journals.length === 0 ? (
        <div className="rounded-[20px] border border-[color:var(--border)] bg-[color:var(--surface)] p-6 text-center shadow-[0_18px_55px_rgba(18,22,34,0.07)]">
          <p className="text-[14px] font-bold text-[color:var(--text)]/52">No journal entries yet.</p>
        </div>
      ) : null}
      ...
      {journals.map((journal) => {
        ...
        return (
          <article
            className="flex flex-col gap-3 rounded-[20px] border border-[color:var(--border)] bg-[color:var(--surface)] p-4 shadow-[0_18px_55px_rgba(18,22,34,0.07)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_24px_72px_rgba(18,22,34,0.10)] sm:flex-row sm:items-start sm:justify-between sm:p-5"
            key={journal.id}
          >
            <div className="space-y-3">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <p className="text-[11px] font-black uppercase tracking-normal text-[#ff4f0a]">
                    {formatJournalDate(journal.publishedAt)}
                  </p>
                  <h3 className="text-[23px] font-black leading-none text-[color:var(--text)] sm:text-[30px]">{journal.title}</h3>
                  {journal.hasPhoto ? <p className="text-[12px] font-bold text-[color:var(--text)]/36">Has photo</p> : null}
                </div>
                {journal.hasPhoto ? (
              ...
              <p className="max-w-3xl whitespace-pre-wrap text-[13px] font-medium leading-6 text-[color:var(--text)]/56">
                {journal.content}
              </p>
            </div>
