...
export function ProjectList({ onReorder, projects }: ProjectListProps) {
  ...
  return (
    <div className="space-y-3">
          ...
          >
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <span className="text-[12px] font-black text-black/22 transition group-hover:text-[#ff4f0a]">::</span>
                <p className="text-[11px] font-black uppercase tracking-normal text-[#ff4f0a]">{project.category}</p>
              </div>
            ...
            <div className="flex shrink-0 items-center gap-3">
              <Link
                className="rounded-full bg-black px-3.5 py-2 text-[12px] font-black text-white shadow-[0_10px_24px_rgba(0,0,0,0.12)] transition hover:-translate-y-0.5 hover:bg-[#ff4f0a]"
              ...
              <form action={deleteAction}>
                <button
                  className="rounded-full border border-black/10 bg-[#ecefed] px-3.5 py-2 text-[12px] font-black text-black/52 transition hover:border-[#ff4f0a]/30 hover:text-[#ff4f0a]"
                  type="submit"
                >
                  Delete
                </button>
      ...
      {error ? <p className="text-sm font-bold text-[#ff4f0a]">{error}</p> : null}
    </div>
  );
}
