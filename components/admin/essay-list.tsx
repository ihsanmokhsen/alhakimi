        ...
        return (
          <article className="flex gap-4 border border-[color:var(--border)] bg-[color:var(--surface)] p-4 shadow-[0_18px_55px_rgba(18,22,34,0.07)] sm:p-5" key={essay.id}>
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[11px] font-black uppercase text-[#ff4f0a]">{formatJournalDate(essay.publishedAt)}</p>
              ...
              <Link className="mt-3 inline-flex text-[12px] font-black text-[#ff4f0a]" href={`/essays/${essay.slug}`} target="_blank">Lihat essay</Link>
            </div>
            <div className="flex shrink-0 flex-col gap-2">
              <Link
                className="bg-black px-3 py-2 text-center text-[12px] font-black text-white transition hover:bg-[#ff4f0a]"
