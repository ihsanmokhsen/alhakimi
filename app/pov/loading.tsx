  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-black">
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-14 pt-16 sm:px-6 sm:pb-20 sm:pt-24 lg:px-8">
        ...
        <div className="mt-5 h-16 w-80 animate-pulse rounded bg-white/8 sm:h-24 sm:w-[28rem]" />
        <div className="mt-8 h-5 w-full max-w-2xl animate-pulse rounded bg-white/6" />
      </section>
        ...
        <div className="grid grid-cols-1 gap-x-5 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div className="w-full space-y-3" key={i}>
              ...
              />
              <div className="h-4 w-3/4 animate-pulse rounded bg-white/8 px-1" />
            </div>
          ))}
        </div>
      </section>
